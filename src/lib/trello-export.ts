...
import { ensureLabels, createCard } from "@/lib/trello";
...
export interface ExportStoriesToTrelloParams {
  teamId: string;
  specId: string;
  boardId: string;
  listId: string;
...
export async function exportStoriesToTrello(
  params: ExportStoriesToTrelloParams
): Promise<TrelloExportResult> {
  const { teamId, specId, boardId, listId } = params;
  ...
  if (!integration) {
    throw new Error("No Trello integration found for this workspace");
  }
  ...
  const { accessToken, accessTokenSecret } = integration;

  // Ensure MoSCoW labels exist on the board
  let labelMap: Record<"MUST" | "SHOULD" | "COULD" | "WONT", string>;
  try {
    labelMap = await ensureLabels(accessToken, accessTokenSecret, boardId);
  } catch {
    throw new Error("Failed to create MoSCoW labels on Trello board");
  }
  ...
  for (const story of stories) {
    try {
      ...
      await createCard(
        ...
        {
          name: story.title,
          desc: buildCardDesc(story, matched),
          priority: story.priority,
        },
        labelMap[story.priority]
      );
      created++;
