...
export async function GET(_request: NextRequest) {
  ...
  if (!process.env.CLICKUP_CLIENT_ID) {
    return new Response("ClickUp integration not configured", { status: 500 });
  }
  ...
  const redirectUri =
    ...
    `${process.env.NEXT_PUBLIC_APP_URL}/api/integrations/clickup/callback`;

  const state = Buffer.from(user.activeWorkspaceId).toString("base64url");

  const authorizeUrl = new URL("https://app.clickup.com/api");
  authorizeUrl.searchParams.set("client_id", process.env.CLICKUP_CLIENT_ID);
